...
type RestOf<T extends IFileUpload | NewFileUpload> = Omit<T, 'uploadDate'> & {
...
export class FileUploadService {
  ...
  protected convertDateFromClient<T extends IFileUpload | NewFileUpload | PartialUpdateFileUpload>(fileUpload: T): RestOf<T> {
    return {
      ...fileUpload,
      uploadDate: fileUpload.uploadDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restFileUpload: RestFileUpload): IFileUpload {
    return {
      ...restFileUpload,
      uploadDate: restFileUpload.uploadDate ? dayjs(restFileUpload.uploadDate) : undefined,
    };
  }
